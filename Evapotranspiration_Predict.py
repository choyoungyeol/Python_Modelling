...
def run_model(selected_features):
    # 엑셀 파일로부터 데이터 불러오기
    data = pd.read_excel('data_prediction.xlsx')

    # 선택된 항목 데이터 추출
    X = data[selected_features]

    # 학습 데이터로 모델 훈련
    train_X = X[:-168]  # 마지막 7일을 제외한 데이터로 학습
    ...
    # 예측 데이터로 예측 수행
    prediction_X = X[-168:]  # 마지막 7일 데이터로 예측
    predictions = model.predict(prediction_X)

    # 그래프 그리기
    plt.rcParams.update({'font.size': 14})
    plt.plot(range(len(train_y)), train_y, label='Measured Value', color='blue')
    plt.plot(range(len(train_y), len(train_y) + len(predictions)), predictions, label='Predicted Value', color='red')
    plt.xlabel('Data')
    plt.ylabel('Evapotranspiration')
    plt.title('Comparison of Measured and Predicted Value')
    plt.legend()
    plt.show()
