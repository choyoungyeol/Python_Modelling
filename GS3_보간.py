...
def animate(i):
    ...
    start_index = max(0, i - 200)  # 최대 200개의 데이터만 보여주기 위해 시작 인덱스를 계산합니다.
    x = data['Date'][start_index:i]  # 현재 인덱스까지의 데이터를 슬라이싱합니다.

    # Plot VWC and Temp
    line_vwc, = plt.plot(x, data['VWC'][start_index:i], 'g.-', label='VWC')
    line_temp, = plt.plot(x, data['Temp'][start_index:i], 'b.-', label='Temp')

    # EC를 제2Y축으로 표시합니다.
    ax2 = plt.twinx()
    ax2.set_ylabel('EC Value', color='m')
    line_ec, = ax2.plot(x, data['EC'][start_index:i], 'm.-', label='EC')
